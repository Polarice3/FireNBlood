...
public class CultistsSpawner {
    ...
    public int tick(ServerWorld world) {
        ...
        if (!FNBConfig.CultistsSpawn.get()) {
            return 0;
        } else {
    ...
    private boolean spawnFanatics(ServerWorld worldIn, BlockPos p_222695_2_, Random random) {
        ...
        if (!WorldEntitySpawner.func_234968_a_(worldIn, p_222695_2_, blockstate, blockstate.getFluidState(), ModEntityType.FANATIC.get())) {
            return false;
        } else if (!PatrollerEntity.func_223330_b(ModEntityType.FANATIC.get(), worldIn, SpawnReason.PATROL, p_222695_2_, random)) {
            return false;
        } else {
    ...
    private boolean spawnWitch(ServerWorld worldIn, BlockPos p_222695_2_, Random random) {
        BlockState blockstate = worldIn.getBlockState(p_222695_2_);
        if (!WorldEntitySpawner.func_234968_a_(worldIn, p_222695_2_, blockstate, blockstate.getFluidState(), EntityType.WITCH)) {
            return false;
        } else if (!PatrollerEntity.func_223330_b(EntityType.WITCH, worldIn, SpawnReason.PATROL, p_222695_2_, random)) {
            return false;
        } else {
            int random1 = worldIn.rand.nextInt(2);
