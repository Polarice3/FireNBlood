...
public class CallerEntity extends AbstractTaillessEntity implements IChargeableMob {
    ...
    public void onDeath(DamageSource cause) {
        for (PlayerEntity entity : this.world.getEntitiesWithinAABB(PlayerEntity.class, this.getBoundingBox().grow(64.0D, 64.0D, 64.0D), field_213690_b)) {
            EffectInstance effectinstance1 = entity.getActivePotionEffect(RegistryHandler.EVIL_EYE.get());
            if (effectinstance1 == null) {
                ...
                entity.removeActivePotionEffect(RegistryHandler.EVIL_EYE.get());
    ...
    class MobSpawnGoal extends Goal {
        ...
        public void tick() {
            ...
            int random2 = world.rand.nextInt(4);
            ...
            for (Entity entity : CallerEntity.this.world.getEntitiesWithinAABB(LivingEntity.class, CallerEntity.this.getBoundingBox().grow(64.0D, 64.0D, 64.0D), field_213690_b)) {
                if (entity instanceof ZombifiedPiglinEntity && target != null) {
                    ((ZombifiedPiglinEntity) entity).setAttackTarget(target);
                }
                if (entity instanceof ZombifiedPiglinEntity && ((ZombifiedPiglinEntity) entity).getAttackTarget() == CallerEntity.this){
                    ((ZombifiedPiglinEntity) entity).setAttackTarget(null);
                    ((ZombifiedPiglinEntity) entity).setAngerTarget(null);
                    ((ZombifiedPiglinEntity) entity).setAngerTime(0);
                }
            }
            ++wavetimer;
                ...
                if (wavetimer >= waveSpawn) {
                    ...
                    if (random == 7 || random == 9) {
                        if (random2 == 1){
                            ...
                            blackBullEntity.enablePersistence();
                            blackBullEntity.setItemStackToSlot(EquipmentSlotType.MAINHAND, new ItemStack(RegistryHandler.GOLDEN_MACE.get()));
                            blackBullEntity.spawnExplosionParticle();
                            world.addEntity(blackBullEntity);
                        } else {
                            BlackBullEntity blackBullEntity = new BlackBullEntity(ModEntityType.BLACK_BULL.get(), world);
    ...
    class MeteorGoal extends Goal{
        ...
        public void tick() {
            ++this.meteorTimer;
            if (!CallerEntity.this.isCharged()){
                this.meteorTimerEnd = 3600 + world.rand.nextInt(3600);
            } else {
                this.meteorTimerEnd = 3600 + world.rand.nextInt(1200);
            }
            if (this.meteorTimer >= meteorTimerEnd){
                ...
                NetherBallEntity fireball = new NetherBallEntity(CallerEntity.this.world, CallerEntity.this, d2, d3, d4);
